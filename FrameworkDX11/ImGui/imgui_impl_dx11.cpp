...
static ID3D11DeviceContext*     _pd3dDeviceContext = NULL;
static IDXGIFactory*            g_pFactory = NULL;
static ID3D11Buffer*            g_pVB = NULL;
static ID3D11Buffer*            g_pIB = NULL;
static ID3D11VertexShader*      _pVertexShader = NULL;
static ID3D11InputLayout*       g_pInputLayout = NULL;
static ID3D11Buffer*            g_pVertexConstantBuffer = NULL;
static ID3D11PixelShader*       _pPixelShader = NULL;
static ID3D11SamplerState*      g_pFontSampler = NULL;
static ID3D11ShaderResourceView*g_pFontTextureView = NULL;
static ID3D11RasterizerState*   g_pRasterizerState = NULL;
static ID3D11BlendState*        g_pBlendState = NULL;
static ID3D11DepthStencilState* _pDepthStencilState = NULL;
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (!g_pVB || g_VertexBufferSize < draw_data->TotalVtxCount)
    {
        ...
        desc.MiscFlags = 0;
        if (_pd3dDevice->CreateBuffer(&desc, NULL, &g_pVB) < 0)
            return;
    }
    if (!g_pIB || g_IndexBufferSize < draw_data->TotalIdxCount)
    {
        ...
        desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
        if (_pd3dDevice->CreateBuffer(&desc, NULL, &g_pIB) < 0)
            return;
    }
...
static void ImGui_ImplDX11_CreateFontsTexture()
{
    ...
    // Upload texture to graphics system
    {
        ...
        _pd3dDevice->CreateShaderResourceView(pTexture, &srvDesc, &g_pFontTextureView);
        pTexture->Release();
    }
    ...
    // Create texture sampler
    {
        ...
        desc.MinLOD = 0.f;
        desc.MaxLOD = 0.f;
        _pd3dDevice->CreateSamplerState(&desc, &g_pFontSampler);
    }
}
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Create the vertex shader
    {
        ...
        if (_pd3dDevice->CreateInputLayout(local_layout, 3, vertexShaderBlob->GetBufferPointer(), vertexShaderBlob->GetBufferSize(), &g_pInputLayout) != S_OK)
        {
            vertexShaderBlob->Release();
            return false;
        }
        ...
        // Create the constant buffer
        {
            ...
            desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
            desc.MiscFlags = 0;
            _pd3dDevice->CreateBuffer(&desc, NULL, &g_pVertexConstantBuffer);
        }
    }
    ...
    // Create the blending setup
    {
        ...
        _pd3dDevice->CreateBlendState(&desc, &g_pBlendState);
    }
    ...
    // Create the rasterizer state
    {
        ...
        _pd3dDevice->CreateRasterizerState(&desc, &g_pRasterizerState);
    }
...
bool    ImGui_ImplDX11_Init(ID3D11Device* device, ID3D11DeviceContext* device_context)
{
    ...
    if (device->QueryInterface(IID_PPV_ARGS(&pDXGIDevice)) == S_OK)
        if (pDXGIDevice->GetParent(IID_PPV_ARGS(&pDXGIAdapter)) == S_OK)
            ...
            {
                _pd3dDevice = device;
                _pd3dDeviceContext = device_context;
                g_pFactory = pFactory;
            }
