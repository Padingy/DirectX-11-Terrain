...
DrawableGameObject::DrawableGameObject(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pContext)
{
	...
	mesh = OBJLoader::Load("Objects/Boat.obj", pd3dDevice);
...
HRESULT DrawableGameObject::initMesh(ID3D11Device* pd3dDevice, ID3D11DeviceContext* pContext)
{
	...
	hr = CreateDDSTextureFromFile(pd3dDevice, L"Resources\\aglis-6plee.dds", nullptr, &m_pTextureResourceView);
	if (FAILED(hr))
		return hr;
...
void DrawableGameObject::update(float t, ID3D11DeviceContext* pContext)
{
	...
	XMMATRIX mTranslate = XMMatrixTranslation(0.0f, -1.0f, -3.0f);
